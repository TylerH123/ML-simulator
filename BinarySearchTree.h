...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
    ...
    if (less(item,node->datum)){ //less than 
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    ...
    if (find_impl(node->right, node->datum, less) != nullptr ||
        find_impl(node->left, node->datum, less) != nullptr) 
      return false;
    if(node->left != nullptr){
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    ...
    if (less(node->datum, val) || 
       (!less(node->datum, val) && !less(val, node->datum))) { //less than or equal to
      ...
      if (less(val,max_element_impl(node->left)->datum)){
